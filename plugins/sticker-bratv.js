...
if (frase.length > 30) return await m.react('❌'), conn.reply(m.chat, `🍓 El texto no puede tener más de 30 caracteres.\n${MARCA}`, m) // <- TU MARCA
await m.react('⏳')
const quoteObj = { type: 'quote', format: 'png', backgroundColor: '#FFB6C1', width: 512, height: 768, scale: 2, messages: [{ entities: [], avatar: true, from: { id: 1, name: nombre, photo: { url: pp } }, text: frase, replyMessage: {} }]}
...
if (!stickerData) return await m.react('❌'), m.reply(`🍓 No se pudo descargar el sticker.\n${MARCA}`) // <- TU MARCA
